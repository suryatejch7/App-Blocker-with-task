...
class AppBlockingService : AccessibilityService() {
    ...
    companion object {
        ...
        fun updateRestrictions(apps: List<String>, websites: List<String>, active: Boolean) {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        when (event.eventType) {
            AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED -> {
                ...
                // Don't block the habit tracker app itself
                ...
                if (restrictedApps.contains(packageName)) {
                    android.util.Log.d("AppBlockingService", "🚫 BLOCKING APP: $packageName")
                    blockApp()
                    return
                } else {
                ...
                if (isBrowserApp(packageName)) {
                    ...
                    // Browser blocking will be handled by checking URL
                    // For now, we'll block all browsers when websites are restricted
                    if (restrictedWebsites.isNotEmpty()) {
                        android.util.Log.d("AppBlockingService", "🚫 BLOCKING BROWSER (has restricted websites)")
                        blockApp()
                    }
                }
            }
